...
pub type PIO_COMPLETION_ROUTINE = Option<extern "fastcall" fn (DeviceObject: PDEVICE_OBJECT, Irp: PIRP, Context: *mut km_void)>;

extern "fastcall"
{
	pub fn IoCompleteRequest(Irp: PIRP, PriorityBoost: KPRIORITY_BOOST);
    pub fn IoSetCompletionRoutine(Irp: PIRP, CompletionRoutine: PIO_COMPLETION_ROUTINE, Context: *const km_void, InvokeOnSuccess: bool, InvokeOnError: bool, InvokeOnCancel: bool);
...
impl IRP {
    ...
    pub fn skip_current_stack_location(&mut self) {
        ...
        unsafe { self.Overlay.CurrentStackLocation = self.Overlay.CurrentStackLocation.offset(1); }
    }
    ...
    pub fn copy_current_stack_location_to_next(&mut self) {
        let curr = self.get_current_stack_location() as *const IO_STACK_LOCATION;
        let next = self.get_next_stack_location();
        unsafe { core::ptr::copy(curr, next, offset_of!(IO_STACK_LOCATION, CompletionRoutine)); }
        next.Control = 0;
    }
